...
static long bg_freq=0;
static long bg_len=0;
	...
	while (TRUE) {
		...
		static struct option long_options[] = {
			...
			{ "max", required_argument, NULL, 'M' },
                        { "bg_freq", required_argument, NULL, 'B'},
                        { "bg_len", required_argument, NULL, 'L'},
		...
		case 'M':
			...
			break;
                case 'B':
                  bg_freq=strtol(optarg,&ptr,10);
                  if ((*optarg == 0) || (*ptr != 0) || (bg_freq<0)) {
                    fprintf(stderr, "Bad argument: %s\n",optarg);
                    fprintf(stderr, TRYFORHELP);
                    exit(1);
                  }
                  break;
                case 'L':
                  bg_len=strtol(optarg,&ptr,10);
                  if ((*optarg == 0) || (*ptr != 0) || (bg_len<0)) {
                    fprintf(stderr, "Bad argument: %s\n",optarg);
                    fprintf(stderr, TRYFORHELP);
                    exit(1);
                  }
