...
def count_kmers(k, genome_fp, cwd, threshold=1):
    ...
    :param threshold: the minimum k-mer frequency (below this will be discarded)
    '''
...
def max_sequential_nt(kmer1, kmer2):
    ...
    kmer1 = mker1.ljust(kmer1_len + len(kmer2), "_")

    max_bind = 0
    for offset in range(kmer1_len):
        consecutive = 0
        for x in range(len(kmer2)):
            if binding[kmer1[offset+x]] == kmer2[x]:
                consecutive += 1
