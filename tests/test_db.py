...
def test_version_check(initdb):
    ...
    with pytest.raises(SystemExit):
        database.check_version(swga_version="0.3.0")


def test_create_primers_sets(initdb, tprimers, tset):
    '''
...
def test_destroy_primer_sets(initdb, tprimers, tset):
    '''
...
def test_add_set_function(initdb, tprimers):
...
def test_bad_add_set_function(initdb, tprimers):
...
@pytest.mark.xfail
def test_update_in_chunks(initdb, tprimers, seqs):
...
def test_primer_updates(initdb, tprimers):
    for primer in tprimers:
        primer.update_tm()
