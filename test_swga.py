...
import swga
from swga.primers import Primer, parse_primer, read_primer_file
from swga.heterodimer_graph import write_graph, test_pairs
from swga.commands.filter_primers import filter_primers
from swga.commands.find_sets import find_sets
class PrimersTests(unittest.TestCase):
    '''
    ...
    def test_primer_parsing(self):
        primer = parse_primer("""ATGC 100 200 0.5""")
        self.assertEqual(primer, Primer(1, "ATGC", 200, 100, 0.5))

    def test_read_primer_file(self):
        infile = StringIO("""AAAA 1 2 3
TTTT 1 2 3 4
GGGG 1 2 3""")
        primers = read_primer_file(infile, False, False)
        self.assertEqual([Primer(1, "AAAA", 2, 1, 3.0),
                          Primer(3, "GGGG", 2, 1, 3.0)], primers)
...
""")
    ...
    def test_filter_primers(self):
        correct_result = [Primer(2, "ATGC", 99, 50, 0.5),
                          Primer(4, "CGTA", 98, 100, 0.2)]
...
class GraphTests(unittest.TestCase):
    ...
    '''
    def setUp(self):
        self.good_set = [Primer(1, "ATGC", 1400, 15, 0),
                         Primer(2, "GGCC", 1500, 10, 0),
                         Primer(3, "CCTA", 2, 4, 0)]
...
'''
        ...
        set_finder = os.path.join(self.swgahome, 'bin/set_finder')
...
class HeterodimerTests(unittest.TestCase):
    ...
    def setUp(self):
        self.primers = []
        self.primers.append(Primer(1, "ATGCTC", 0, 0, 0))
        # 4 contiguous
        self.primers.append(Primer(2, "CAGCAT", 0, 0, 0))
        # 3 contiguous, alt
        self.primers.append(Primer(3, "GAGGTA", 0, 0, 0))
        # 3 contiguous, alt 2
        self.primers.append(Primer(4, "ATCGAC", 0, 0, 0))
        # valid
        self.primers.append(Primer(5, "TTCCAC", 0, 0, 0))


    def test_compatible_primers(self):
        edges = test_pairs(self.primers[0:2], 3)
        self.assertEqual(edges, [])
        edges = test_pairs([self.primers[0], self.primers[4]], 3)
...
class PrimerBindingLocationTests(unittest.TestCase):
    ...
    def test_find_locations(self):
        test_str = "TATATATAT"
        substr = "TAT"
        locs = swga.genome.find_locations(substr, test_str)
    ...
    def test_find_primer_locations(self):
        ...
        with tempfile.NamedTemporaryFile() as tmp:
            ...
            tmp.flush()
            filename = tmp.name
            primer = Primer(1, "TATA", 0, 0, 0)
            _, test_locs = swga.genome.find_primer_locations(primer, filename)
